...
//
//	SimpleList<MatrixNode*> * list = pathfinding->DistanceFirstSearch(start,
//			goal);
...
void textImage()
{
	detect = new ImageVideoDetection();
	IplImage* img = cvLoadImage("image.jpg");
	detect->matrixImage(img);
	cvNamedWindow("Original", CV_WINDOW_AUTOSIZE);
	cvShowImage("Original", img);

	cvNamedWindow("Reconstruccion", CV_WINDOW_AUTOSIZE);
	cvShowImage("Reconstruccion", detect->reconstructImage());
